...
public class MenuArrivalUpdate implements Renderer {
	...
	public void render() {
		...
		try {
			...
			while (!fechaOk) {
				fechaArrival = Keyin.inString("Fecha de Arribo ("+sdf.format(arrival.getArrivalDate())+"): ");
			...
			String shipOrigin =  Keyin.inString("Pais de origen ("+arrival.getShipOrigin()+"): ");
			...
			String containersDescriptions 	= Keyin.inString("Descipcion de Contenedores ("+arrival.getContainersDescriptions()+"): ");
			List<Long> containers 		= new ArrayList<>();
			
			int incambiaCont = Keyin.inInt("Cambiar los contenedores (1 si / 0 no)?");
			if(incambiaCont == 1){
				System.out.println("Contenedores ("+generateContainerList(arrival.getContainers())+"), \nse deben ingresar nuevamente:");
			...
			if (shipOrigin != null && !shipOrigin.isEmpty()
					&& !arrival.getShipOrigin().trim().equals(shipOrigin.trim())) {
				arrival.setShipOrigin(shipOrigin);
				hayCambios = true;
			}			
			if (containersDescriptions != null && !containersDescriptions.trim().isEmpty()
					&& !arrival.getContainersDescriptions().trim().equals(containersDescriptions.trim())) {
			...
			if (!hayCambios) {
				...
				while (!exit) {
					...
					case 'S':
						try {
							exit = true;
							long idGenerated = client.update(arrival, shipid, containers);
